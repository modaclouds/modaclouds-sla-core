...
public class Tower4CloudsTranslatorTest {
	...
	public void testTranslate() throws IOException {
		...
		Tower4CloudsTranslator translator = new Tower4CloudsTranslator(
				constraintEvaluator);
		...
		agreement.setGuaranteeTerms(Arrays
				.asList(new IGuaranteeTerm[] { term }));

		Map<IGuaranteeTerm, List<IMonitoringMetric>> map = translator
				.translate(agreement, json);
		...
		assertEquals("AverageDatabaseCPUUtilizationViolated",
				item.getMetricKey());
		assertEquals("0.022784559188465933e0", item.getMetricValue());
	}

	@Test
	public void testTranslate2() throws IOException {

		String json = readFile("/violation_t4c_2.json");
		IConstraintEvaluator constraintEvaluator = new SimpleConstraintEvaluator();
		...
		term.setKpiName("responsetime");
		term.setName("rt");
		term.setServiceLevel("ResponseTimeLogin_login_violated NOT_EXISTS");
		agreement.setGuaranteeTerms(Arrays
				.asList(new IGuaranteeTerm[] { term }));
		Map<IGuaranteeTerm, List<IMonitoringMetric>> map = translator
				.translate(agreement, json);
