 ...
 * @author rsosa
 */
public class AgreementEnforcement implements IAgreementEnforcement {
	...
	private IAgreementEvaluator agreementEval;
	...
	@Override
	public void enforce(IAgreement agreement, Date since) {
		...
		Map<IGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem> retrievalItems = 
				buildRetrievalItems(agreement, since, now);
	...
	@Override
	public void enforce(IAgreement agreement,
			Map<IGuaranteeTerm, List<IMonitoringMetric>> metricsMap) {
		logger.debug(String.format("enforce(agreement=%s)", agreement.getAgreementId()));
...
//		service.saveEnforcementResult(agreement, evaluationResult);
	...
	private void checkInitialized(boolean checkRetriever) {
		if (agreementEval == null) {
			throw new NullPointerException("agreementEval has not been set");
		}
	...
	private Map<IGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem> buildRetrievalItems(
			...
			final Date since,
			final Date now) {
		logger.debug("buildRetrievalItems()");
	...
	private Map<IGuaranteeTerm, List<IMonitoringMetric>> getMetrics(
			...
			IAgreement agreement, 
			Map<IGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem> retrievalItems) {
		logger.debug("getMetrics(IMetricsRetriever)");
	...
	public IAgreementEvaluator getAgreementEvaluator() {
	...
	public int getMaxRetrievedResults() {
		return maxRetrievedResults;
	}
