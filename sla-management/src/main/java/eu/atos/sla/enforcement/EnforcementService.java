...
public class EnforcementService implements IEnforcementService {
	...
	@Autowired
	IAgreementEnforcement agreementEnforcement;
	...
	public void saveEnforcementResult(IAgreement agreement,
		...
		for (IGuaranteeTerm term : result.keySet()) {
			GuaranteeTermEvaluationResult termResult = result.get(term);
			
			for (IViolation violation : termResult.getViolations()) {

				violationDAO.save(violation);
				term.getViolations().add(violation);
			}
			
			for (@SuppressWarnings("unused") ICompensation compensation : termResult.getCompensations()) {
...
//				term.getCompensations().add(compensation);
			...
			term.setStatus( term.getViolations().size() > 0? 
